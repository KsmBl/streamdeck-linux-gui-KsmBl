...
* **Per-page Application Binding**: A page can be shown automatically when a chosen application is focused, so it can present actions for whatever you're using. Open the page's settings (the gear button above the pages) and set its application — pick a running app from the list or type its id. The feature activates as soon as any page has an application set. Detection works on X11, Sway and Hyprland out of the box (and KDE with `kdotool`); some Wayland compositors such as GNOME don't expose the focused window, where it stays inactive.
