...
* **Follow Focused Application**: Automatically switch pages based on the focused application — so a page can show actions for whatever you're using. Enable **View → Follow Focused Application**, then use **View → Assign Page to Focused App…** to bind the current page to the focused app (and **Clear Page's Focus App** to remove it). Works on X11, Sway and Hyprland out of the box (and KDE with `kdotool`); some Wayland compositors such as GNOME don't expose the focused window, where the feature stays inactive.
