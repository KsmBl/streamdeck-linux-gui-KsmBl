...
* **Fokussierter Anwendung folgen**: Wechselt auf Wunsch automatisch die Seite je nach fokussierter Anwendung – so kann eine Seite Aktionen für das gerade genutzte Programm anzeigen. Aktivieren über **View → Follow Focused Application**, dann mit **View → Assign Page to Focused App…** die aktuelle Seite an die fokussierte Anwendung binden (und mit **Clear Page's Focus App** wieder entfernen). Funktioniert unter X11, Sway und Hyprland (sowie KDE mit `kdotool`); manche Wayland-Compositoren wie GNOME geben das fokussierte Fenster nicht preis, dort bleibt die Funktion inaktiv.
